if(global.woodSelected && !obj_runbtn.physics_running){
	if(mouse_check_button_pressed(mb_left)){
		...
		beam = instance_create_depth(start_nearest_joint.x, start_nearest_joint.y-4, 0, obj_woodbeam);
	...
	if(mouse_check_button_released(mb_left)){
		...
		if(instance_nearest(mouse_x,mouse_y,obj_joint) != start_nearest_joint){
			...
			end_nearest_joint = instance_nearest(mouse_x+sprite_get_width(spr_wood),mouse_y,obj_joint);
		}
