if(global.stoneSelected && !obj_runbtn.physics_running){
	if(mouse_check_button_pressed(mb_left)){
		show_debug_message("Getting starting point for stone beam");
		...
		beam = instance_create_depth(start_nearest_joint.x, start_nearest_joint.y-4, 0, obj_stonebeam);
		ds_list_add(start_nearest_joint.connections,beam);
	}
	if(mouse_check_button_released(mb_left)){
		show_debug_message("Getting end point for stone beam");
		if(instance_nearest(mouse_x,mouse_y,obj_joint) != start_nearest_joint){
			...
			end_nearest_joint = instance_nearest(mouse_x+sprite_get_width(spr_stone),mouse_y,obj_joint);
		}
